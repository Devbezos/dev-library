...
namespace dev_library.Data
{
    ...
    public interface ITcgProductGroupRepository
    {
        ...
        void SetMarketPrices(string game, string groupKey, string displayName, decimal? msrp, decimal? resaleMarketPrice);
    }
    ...
    public class TcgProductGroupRepository : ITcgProductGroupRepository
    {
        ...
        public List<TcgProductGroup> GetAll(string? game = null)
        {
            ...
            using var cmd = conn.CreateCommand();
            cmd.CommandText = """
                SELECT game, group_key, display_name, msrp, resale_market_price
            ...
            while (reader.Read())
            {
                groups.Add(new TcgProductGroup
                {
                    ...
                    Msrp = reader["msrp"] == DBNull.Value ? null : reader.GetDecimal("msrp"),
        ...
        public void SetMarketPrices(string game, string groupKey, string displayName, decimal? msrp, decimal? resaleMarketPrice)
        {
            using var conn = new MySqlConnection(_connectionString);
            conn.Open();
            ...
            using var cmd = conn.CreateCommand();
            cmd.CommandText = """
                INSERT INTO tcg_product_groups (game, group_key, display_name, msrp, resale_market_price)
                VALUES (@game, @groupKey, @displayName, @msrp, @resaleMarketPrice)
                ON DUPLICATE KEY UPDATE display_name = @displayName, msrp = @msrp, resale_market_price = @resaleMarketPrice
                """;
            ...
            cmd.Parameters.AddWithValue("@msrp", (object?)msrp ?? DBNull.Value);
