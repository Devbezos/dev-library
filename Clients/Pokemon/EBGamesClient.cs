...
namespace dev_library.Clients
{
    ...
    {
        private static readonly string EBGamesUrl = "https://www.ebgames.ca/SearchResult/QuickSearch?q=Pok%C3%A9mon%20&platform=361&rootGenre=99&shippingMethod=1&release=1&page={0}";
        private static readonly string EBGamesBaseUrl = "https://www.ebgames.ca";
        ...
        public async Task<List<Search>> GetPokemon()
        {
            ...
            {
                Log.Information("EBGamesClient.GetProducts: END — {Count} total products", allProducts.Count);
            }

            return allProducts.Count > 0
                ? new List<Search> { new Search("Pokemon", "EBGames", allProducts) }
                : new List<Search>();
        }
    }
}
